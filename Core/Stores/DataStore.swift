...
public final class DataStore<S: Scheduler>: ObservableObject {
	@Published public private(set) var posts: StoreData<[Post.Connected]> = .init()
	@Published public private(set) var users: StoreData<[User.Connected]> = .init()
	@Published public private(set) var comments: StoreData<[Comment.Connected]> = .init()
...
public typealias StoreData<T> = RemoteData<T, FetchError>
