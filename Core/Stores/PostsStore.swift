...
public final class PostsStore<S: Scheduler>: ObservableObject {
	@Published public private(set) var posts: StoreData<[Post]> = .init()
	...
	public typealias GetPosts = () -> AnyPublisher<[Post], FetchError>
}

public extension PostsStore where S == RunLoop {
	convenience init(
		storage: Storage,
		getPosts: @escaping GetPosts
	) {
